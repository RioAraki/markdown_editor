...
export function interviewContext(context: InterviewContext, followUps: FollowUp[]): string {
  const lines = [
    '【系统提示词：你的角色与协作方式】', '',
  ...
  if (!active.length) lines.push('暂未记录追问，请基于原问题与我的回答，提出最值得准备的追问。');
  active.forEach((f, i) => {
  ...
  lines.push('', '五、我希望你现在怎样帮助我', '',
    '先简要说明：为什么读到这段简历、听到我的回答之后，面试官会提出这些追问？',
    '然后从尚未准备或磕磕绊绊的追问中选一个最关键的，和我深入讨论。如果由你来负责，你会如何设计和实现整个过程？请优先用我的回答中出现的场景，逐步说明输入、判断、执行、输出、失败处理和验证方式；若需另设场景，请明确标注是假设。',
